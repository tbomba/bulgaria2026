...
const { profile, isAdmin, userTeam, signOut } = useAuth();
...
<template>
  <header
    class="bg-white/[0.03] backdrop-blur-xl shadow-glass sticky top-0 z-50 border-b border-white/[0.06]"
  >
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-12 sm:h-16">
        ...
        <NuxtLink to="/" class="flex items-center gap-2 shrink-0">
          ...
          <span
            class="font-heading font-extrabold text-xl text-white hidden sm:inline tracking-tight"
          >
            Bulharsko 2026
          </span>
        </NuxtLink>
        ...
        <div class="flex items-center gap-2 sm:gap-3">
          <span
            v-if="profile"
            class="text-sm text-neutral-400 hidden lg:inline"
          >
    ...
    >
      <nav
        v-if="mobileMenuOpen"
        class="md:hidden bg-black/60 backdrop-blur-xl border-t border-white/[0.06] px-4 pb-4 pt-2"
      >
        <NuxtLink
          v-for="link in navLinks"
          :key="link.path"
        ...
        >
          <svg
            class="w-4 h-4 inline -mt-0.5 mr-2"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
